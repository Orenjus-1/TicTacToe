...
winner_img_img = pygame.transform.scale(winner_img, (SCREEN_WIDTH, SCREEN_HEIGHT))
# Load sounds
...
def show_end_screen(winner, consecutive_draws=0):
    global player_score, ai_score, single_player_mode
    
    screen.blit(score_img, (0, 0))  # Display the score background
    win_sound.play()

    # Display scores for both single-player and multiplayer
    if single_player_mode:
    ...
    score_rect = score_display.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 50))
    screen.blit(score_display, score_rect)

    # Determine and display the end game message
    if winner:
        if single_player_mode:
    ...
    # Draw buttons for retry and menu
    draw_button_with_text("Retry", (SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 50))
    draw_button_with_text("Menu", (SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 120))

    pygame.display.update()

    # Wait for user input to either retry or return to menu
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 50, 200, 50).collidepoint(mouse_pos):
                    click_sound.play()
                    return True  # Retry
                elif pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 120, 200, 50).collidepoint(mouse_pos):
                    click_sound.play()
                    return False  # Menu

def game_loop():
    global player_turn, game_over, player_choice, ai_choice
    reset_board()
    draw_board()
    pygame.display.update()
    ...
    while not game_over:
                ...
                if board[clicked_row][clicked_col] == "":
                    if single_player_mode:
                        if player_turn:
                            ...
                            draw_board()
                            pygame.display.update()
                            winner = check_winner()  # Check if the player has won
                            if winner == player_choice:
                            ...
                            if not game_over:
                                ...
                                if ai_move:
                                    ...
                                    draw_board()
                                    pygame.display.update()
                                    winner = check_winner()  # Check if the AI has won
                                    if winner == ai_choice:
                                        update_scores(ai_choice)
                                        game_over = True
                                    elif winner == player_choice:
                                        update_scores(player_choice)
                                        game_over = True
                                player_turn = True
                    else:
                        if player_turn:
                            ...
                            draw_board()
                            pygame.display.update()
                            winner = check_winner()  # Check if X has won
                            if winner == "X":
                                update_scores("X")
                                game_over = True
                            player_turn = False
                            ...
                            draw_board()
                            pygame.display.update()
                            winner = check_winner()  # Check if O has won
                            if winner == "O":
                                update_scores("O")
                                game_over = True
                            player_turn = True
        ...
        if game_over:
            if not check_best_of_3():
                retry = show_end_screen(winner)
                if retry:
                    reset_board()
                    game_loop()
                else:
